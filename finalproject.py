...
def fbconnect():
    ...
    'client_secret=%s&fb_exchange_token=%s' % (app_id, app_secret, access_token)  # noqa
    h = httplib2.Http()
...
def gconnect():
    ...
    "-moz-border-radius: 150px;" > flash("you are now logged in as %s" % login_session['username'])  # noqa
    print "done!"
    return output
...
''' function to authenticate users before performing CRUD operations'''
...
def deleteRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteRestaurant.html', restaurant=restaurantToDelete)  # noqa
...
def showMenu(restaurant_id):
    ...
    if creator.id != login_session['user_id']:
        return render_template('publicmenu.html', items=items, restaurant=restaurant, creator=creator)  # noqa
    else:
        return render_template('menu.html', items=items, restaurant=restaurant, creator=creator)  # noqa
...
def newMenuItem(restaurant_id):
    ...
    if request.method == 'POST':
            newItem = MenuItem(name=request.form['name'],
                               ...
                               restaurant_id=restaurant_id, user_id=restaurant.user_id)  # noqa
            session.add(newItem)
            session.commit()
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        ...
        "Please create your own restaurant in order to edit items.');}</script>"  # noqa
        "<body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        return render_template('editmenuitem.html', restaurant_id=restaurant_id, menu_id=menu_id, item=editedItem)  # noqa
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if restaurant.user_id != login_session['user_id']:
        return "<script>function myFunction()"
        "{alert('You are not authorized to delete menu items to this restaurant."  # noqa
        "Please create your own restaurant in order to delete items.');}</script>"  # noqa
        "<body onload='myFunction()''>"
    if request.method == 'POST':
